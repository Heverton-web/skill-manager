...
1. **Estilo Caveman:** Comunicação direta e pensamento telegráfico (máx. 3-5 linhas). Sem saudações ou preâmbulos. Idioma PT-BR.
2. **Headroom & RTK:** Todo log, JSON ou output > 7 linhas DEVE ser comprimido via `headroom` (topo 3 + fim 4) e filtrado via `rtk`.
3. **LeanCTX:** Usar `grep_search` antes de abrir arquivos e limitar a leitura com `StartLine`/`EndLine`.
