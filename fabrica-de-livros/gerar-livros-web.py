#!/usr/bin/env python3
"""
Gerador dos Livros das 5 Séries de Desenvolvimento Web Fullstack (W1-W5) — 50 livros
Gera capítulos com conteúdo real seguindo EITA-V2 para:
  - W1: Fundamentos da Web e Arquitetura (10 livros)
  - W2: Frontend: Interfaces Modernas e Escaláveis (10 livros)
  - W3: Backend: Servidores, APIs e Lógica de Negócio (10 livros)
  - W4: Bancos de Dados e Persistência de Dados (10 livros)
  - W5: DevOps, Automação, IA e Carreira Fullstack (10 livros)
...
Uso: python gerar-livros-web.py
"""
...
# ── CARREGAR 50 LIVROS DAS SÉRIES DE WEB ────────────────────────
try:
    from dados_series_web import LIVROS_WEB, SLUGS_WEB, SERIES_WEB, SERIES_PARTES
except ImportError:
    LIVROS_WEB = {}
    SLUGS_WEB = []
    SERIES_WEB = {}
...
NOMES_LIVROS = {_sl: LIVROS_WEB[_sl][0] for _sl in SLUGS_WEB}

SLUGS = list(SLUGS_WEB)

# ── POOLS DE TEMPLATES VARIADOS (temas de desenvolvimento web) ──

ABORDAGENS_EXPLICA = [
    "ocupa um lugar central no desenvolvimento web moderno. Dominar seus fundamentos e nuances é essencial para qualquer pessoa que deseja construir aplicações robustas, escaláveis e de alta qualidade.",
...
REFS_BASE = [
    "[1] PEREIRA, Heverton Eduardo. *Desenvolvimento Web Fullstack: Fundamentos e Arquitetura*. Fábrica Agêntica de Livros, 2026.",
    ...
    "[7] FIELDING, Roy T. *Architectural Styles and the Design of Network-based Software Architectures*. Tese (Doutorado) — Universidade da Califórnia, Irvine, 2000.",
    ...
    "[10] OWASP Foundation. *OWASP Top 10: The Ten Most Critical Web Application Security Risks*. OWASP, 2021.",
]

REF_EXTRA = [
    "[11] OPENAI. *GPT-4 Technical Report*. arXiv:2303.08774, 2023.",
    "[12] ANTHROPIC. *Claude 3 Model Family*. arXiv:2407.00750, 2024.",
    "[13] KARPUKHIN, V. et al. *Are Pre-trained Convolutions Better than Pre-trained Transformers?*. arXiv:2105.07122, 2021.",
    "[14] FRASER, B.; WAGNER, R. *Web Vitals and Core Web Vitals: Measuring User Experience*. Google Web Fundamentals, 2023.",
    "[15] NEWMAN, Sam. *Building Microservices: Designing Fine-Grained Systems*. 2. ed. Sebastopol: O'Reilly, 2021.",
]
...
def get_capitulos_por_parte(parte_num, serie, tema_livro):
    ...
    # (ex.: "A Anatomia da Web" -> "Anatomia da Web" / "anatomia da web")
    t = tema_livro
...
def gerar_conteudo_capitulo(slug, cap_num, cap_info, sumario):
    ...
    if slug in LIVROS_WEB and cap_num == 1:
        secao_explica = LIVROS_WEB[slug][5]
        secao_ilustra = ""
        secao_tecnica = ""
        secao_aplica = ""
    else:
    ...
    seed = zlib.crc32(f"{slug}-{cap_num}-web".encode("utf-8")) % 10000
...
<!--
  ...
  Slug: {slug}
  Série: {SERIES_WEB.get(slug.split('-')[0], {}).get('nome', '')}
  Capítulos: 16
  Gerado em: {hoje}
-->
"""
...
def gerar_sumario(slug):
    ...
    nome, titulo_obra, subtitulo, introducao, conclusao, _ = LIVROS_WEB[slug]
    serie = slug.split("-")[0]
    ...
    sumario = {
        ...
        "conclusao": conclusao,
        "serie": SERIES_WEB.get(serie, {}).get("nome", serie),
        "partes": partes,
    }
    return sumario
...
def main():
    print("=" * 60)
    print("  GERADOR DAS 5 SÉRIES DE DESENVOLVIMENTO WEB (W1-W5) — 50 LIVROS")
    print("=" * 60)
    print()
