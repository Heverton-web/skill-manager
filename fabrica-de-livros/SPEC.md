...
| R7 | PDF final | 1 arquivo .pdf | Pandoc+Typst (obrigatório) |
| R8 | Linguagem | Simples para iniciantes, transformacional para PhDs | Tom em camadas |
| R9 | Citações inline [N] | Mínimo 3 por capítulo | Vinculadas às referências |
...
└─────────┬──────────────────────┘
   ...
   ╚══════════════════════════════════════════════════════════════╝
          ...
          ▼
┌───────────────────────────────────────┐
│ Passo 3 — Fase 3 (Acabamento & ABNT)  │  compilador-abnt: merge + ABNT → livro_final.md
│                                         │  Nó 10: Pandoc+Typst → livro_final.pdf
└─────────┬───────────────────────────────┘
          ▼
┌──────────────────────┐
...
| | |
|---|---|
| Agentes/Subagentes | `subagente-redator-capitulo` (processamento paralelo) |
| Entrada | Coordenadas `{parte, capitulo}` para capítulos |
...
### Passo 3 — Fase 3 (Acabamento & ABNT) + exportação em PDF
| | |
|---|---|
| Agente | `compilador-abnt` (Nós 5–10) |
| Pré-condição | Todos os capítulos finalizados |
| Saída | `output/<slug>/livro_final.md` (Nó 9) e `output/<slug>/livro_final.pdf` (Nó 10, via Pandoc+Typst) |
