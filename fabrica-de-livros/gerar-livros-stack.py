#!/usr/bin/env python3
"""
Gerador dos Livros das 5 Séries da Stack Fullstack (FE, BE, BD, AP, DV) — 50 livros
Gera capítulos com conteúdo real seguindo EITA-V2 para:
  - FE: Frontend (10 livros)
  - BE: Backend (10 livros)
  - BD: Banco de Dados (10 livros)
  - AP: APIs (10 livros)
  - DV: DevOps (10 livros)
...
Uso: python gerar-livros-stack.py
"""
...
# ── CARREGAR 50 LIVROS DAS SÉRIES DA STACK ─────────────────────
try:
    from dados_series_stack import LIVROS_STACK, SLUGS_STACK, SERIES_STACK, SERIES_PARTES
except ImportError:
    LIVROS_STACK = {}
    SLUGS_STACK = []
    SERIES_STACK = {}
...
NOMES_LIVROS = {_sl: LIVROS_STACK[_sl][0] for _sl in SLUGS_STACK}

SLUGS = list(SLUGS_STACK)

# ── POOLS DE TEMPLATES VARIADOS (temas da stack fullstack) ─────

ABORDAGENS_EXPLICA = [
    "ocupa um lugar central na engenharia de software moderna. Dominar seus fundamentos e nuances é essencial para qualquer pessoa que deseja construir aplicações robustas, escaláveis e de alta qualidade.",
    "é um dos pilares que separam quem apenas escreve código de quem projeta sistemas. Compreendê-lo em profundidade muda a forma como você estrutura cada projeto que inicia.",
    ...
    "funciona como um multiplicador de força para o time: quando bem compreendido, um único conceito ilumina dezenas de decisões de arquitetura e código.",
]

POR_QUE_IMPORTA = [
    "ajusta as variáveis que controlam a qualidade do sistema em escala. Aplicar corretamente reduz o custo de manutenção, previne bugs e acelera o onboarding de novos desenvolvedores.",
...
METAFLUSTRAS = [
    "como um arquiteto que projeta antes de construir: cada viga, cada encanamento e cada tomada é planejado para suportar o peso do futuro. O código bem arquitetado é um prédio que recebe reformas sem desabar.",
    ...
    "como um maestro regendo uma orquestra: cada instrumento precisa entrar no momento exato, no volume certo, para que a sinfonia complete o público. No software, cada módulo e cada serviço também precisam de timing e harmonia.",
...
REFS_BASE = [
    "[1] PEREIRA, Heverton Eduardo. *Stack Fullstack: Fundamentos, Arquitetura e Práticas*. Fábrica Agêntica de Livros, 2026.",
    "[2] MARTIN, Robert C. *Código Limpo: Habilidades Práticas do Agile Software*. Rio de Janeiro: Alta Books, 2009.",
    "[3] FOWLER, Martin. *Refactoring: Improving the Design of Existing Code*. 2. ed. Boston: Addison-Wesley, 2018.",
    "[4] GAMMA, Erich; HELM, Richard; JOHNSON, Ralph; VLISSIDES, John. *Padrões de Projeto: Soluções Reutilizáveis*. Porto Alegre: Bookman, 2000.",
    "[5] CHACON, Scott; STRAUB, Ben. *Pro Git*. 2. ed. Nova York: Apress, 2014.",
    "[6] FIELDING, Roy T. *Architectural Styles and the Design of Network-based Software Architectures*. Tese (Doutorado) — Universidade da Califórnia, Irvine, 2000.",
    "[7] CROCKFORD, Douglas. *JavaScript: The Good Parts*. Sebastopol: O'Reilly, 2008.",
    "[8] OWASP Foundation. *OWASP Top 10: The Ten Most Critical Web Application Security Risks*. OWASP, 2021.",
    "[9] KLEPPMANN, Martin. *Designing Data-Intensive Applications*. Sebastopol: O'Reilly, 2017.",
    "[10] HUNT, Andrew; THOMAS, David. *O Programador Pragmático*. Porto Alegre: Bookman, 2020.",
]

REF_EXTRA = [
    "[11] BASS, Len; CLEMENTS, Paul; KAZMAN, Rick. *Software Architecture in Practice*. 3. ed. Boston: Addison-Wesley, 2012.",
    "[12] NEWMAN, Sam. *Building Microservices: Designing Fine-Grained Systems*. 2. ed. Sebastopol: O'Reilly, 2021.",
    "[13] TURNBULL, James. *The Docker Book: Containerization Is the New Virtualization*. 2014.",
    "[14] HAVERBEKE, Marijn. *Eloquent JavaScript: A Modern Introduction to Programming*. 3. ed. São Francisco: No Starch Press, 2018.",
    "[15] COPELAND, William. *PostgreSQL for Dummies*. Nova Jersey: John Wiley & Sons, 2015.",
]
...
def get_capitulos_por_parte(parte_num, serie, tema_livro):
    ...
    # (ex.: "O Ecossistema TypeScript" -> "Ecossistema TypeScript")
    t = tema_livro
    for art in ["A ", "O ", "Os ", "As ", "Um ", "Uma "]:
        if t.startswith(art):
            t = t[len(art):]
            break
    # minúsculas para meio de frase — preserva siglas iniciais (CSS, HTML5, CI/CD)
    ...
    if parte_num == 1:
        return [
            ...
            {"capitulo": 3, "titulo": f"O Vocabulário Essencial de {t}", "subtitulo": f"Termos e noções que todo desenvolvedor de {tn} precisa conhecer"},
...
def gerar_conteudo_capitulo(slug, cap_num, cap_info, sumario):
    ...
    if slug in LIVROS_STACK and cap_num == 1:
        secao_explica = LIVROS_STACK[slug][5]
        secao_ilustra = ""
        secao_tecnica = ""
        secao_aplica = ""
    else:
    ...
    seed = zlib.crc32(f"{slug}-{cap_num}-stack".encode("utf-8")) % 10000
    ...
    if not secao_explica:
        ...
        secao_explica = (
            ...
            f"- Desenvolvedores de {nome_livro.lower()} em diferentes níveis\n"
            f"- Momentos de arquitetura, implementação e operação de sistemas\n"
            f"- Estratégias práticas para entregar software de qualidade\n\n"
...
<!--
  ...
  Slug: {slug}
  Série: {SERIES_STACK.get(slug.split('-')[0], {}).get('nome', '')}
  Capítulos: 16
  Gerado em: {hoje}
-->
"""
...
def gerar_sumario(slug):
    ...
    nome, titulo_obra, subtitulo, introducao, conclusao, _ = LIVROS_STACK[slug]
    serie = slug.split("-")[0]
    ...
    sumario = {
        ...
        "conclusao": conclusao,
        "serie": SERIES_STACK.get(serie, {}).get("nome", serie),
        "partes": partes,
    }
    return sumario
...
def main():
    print("=" * 60)
    print("  GERADOR DAS 5 SÉRIES DA STACK FULLSTACK (FE/BE/BD/AP/DV) — 50 LIVROS")
    print("=" * 60)
    print()
