#!/usr/bin/env python3
"""
Compila cada serie MK1-MK5 (20 livros cada) em um mega-livro unico por serie.
Seguindo o fluxo de compilar-mega-camadas.py / compilar-mega-livro.py.

Fluxo (por serie):
  1. Criar pasta limpa output/MKx-mega-serie-MKx/
...
Uso: python compilar-mega-planejamento.py
"""
...
try:
    from dados_series_planejamento import (
        SLUGS_PLANEJAMENTO,
        SERIES_PLANEJAMENTO,
        LIVROS_PLANEJAMENTO,
    )
except ImportError:
    print("[ERRO] dados_series_planejamento.py nao encontrado (leve os arquivos mk3/mk4/mk5 juntos).")
    sys.exit(1)
...
def compilar_serie(prefix, hoje):
    ...
    serie_nome = SERIES_PLANEJAMENTO.get(prefix, {}).get("nome", prefix)
    slugs_serie = [s for s in SLUGS_PLANEJAMENTO if s.split("-")[0] == prefix]
    ...
    introducao = (
        ...
        f"{serie_nome}. Cada parte corresponde a um livro original da série, "
        "mantendo sua identidade e estrutura interna, com os capítulos renumerados "
        "sequencialmente para facilitar a navegação e a referência cruzada."
    )
    conclusao = (
        f"A série {prefix} — {serie_nome} — entrega um percurso completo e "
        "sequencial sobre o tema: dos fundamentos conceituais às práticas "
        "avançadas de execução e mensuração. Este compilado em volume único "
    ...
    for idx, (slug, sumario) in enumerate(sumarios):
        ...
        if not titulo_parte:
            titulo_parte = LIVROS_PLANEJAMENTO.get(slug, ("", "", "", "", "", ""))[1]
        if not titulo_parte:
            titulo_parte = slug
...
Cada parte corresponde a um livro original da série, mantendo sua identidade e estrutura interna.
...
def main():
    hoje = date.today()
    step("MEGA-LIVROS POR SERIE (MK1-MK5)")

    resultados = []
    falhas = []

    for prefix in sorted(SERIES_PLANEJAMENTO.keys()):
    ...
    print(f"  Mega-livros criados: {len(resultados)}/{len(SERIES_PLANEJAMENTO)}")
